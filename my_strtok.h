//
// Created by Iliya on 21.11.2019.
//

void skipSeps(char** str, char* sep);
void skipNonSeps(char** str, char* sep);
...
char* my_strtok(char* str, char*sep) {
    ...
    static char* prev_ptr = nullptr;

    if (str)
        prev_ptr = str; //if new string splitting

    if (prev_ptr == nullptr)
        return nullptr; //in case of inaccurate use

    if (*prev_ptr == '\0')
        return nullptr; // in case of string end
